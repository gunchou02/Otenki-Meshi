...
- **データ駆動:** 料理候補を `backend/foods.json` に分離し、コードを触らずに候補・カテゴリ・重みを追加可能。
- **自然さ:** 上位候補から重み付きランダムで選ぶことで、毎回同じ提案になりすぎないように調整。
- **多様性:** キーワード単位だけでなくカテゴリ単位でも直近推薦を減点し、似たジャンルの連続を抑制。
...
Skill Set: React, Lravel, Python, AWS, Terraform, HTML, CSS, JS, PHP
